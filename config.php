...
$db_name = getenv('DB_NAME') ?: 'u289267434_runlife';
$db_user = getenv('DB_USER') ?: 'u289267434_runlife';
$db_pass = getenv('DB_PASS') ?: 'Zonete@13';


try {
    // Conectar ao banco (a criação do banco deve ser feita no painel do host)
    ...
    $tables = [
        ...
        "CREATE TABLE IF NOT EXISTS life_rules (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT DEFAULT 1, rule_text TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS runs (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT DEFAULT 1, title VARCHAR(255) NOT NULL, run_date DATE NOT NULL, distance_km DECIMAL(6,2) DEFAULT 0, notes VARCHAR(255) DEFAULT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS daily_photos (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT DEFAULT 1, photo_date DATE NOT NULL, photo_url MEDIUMTEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY uniq_user_date (user_id, photo_date))",
        "CREATE TABLE IF NOT EXISTS user_settings (user_id INT PRIMARY KEY, total_xp INT DEFAULT 0)",
        "CREATE TABLE IF NOT EXISTS bets (id INT AUTO_INCREMENT PRIMARY KEY, bet_date DATE NOT NULL, odds DECIMAL(10,2) NOT NULL, stake DECIMAL(10,2) NOT NULL, result ENUM('Green','Red','Void') NOT NULL, profit DECIMAL(10,2) NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS bet_selections (id INT AUTO_INCREMENT PRIMARY KEY, bet_id INT NOT NULL, comp VARCHAR(255) NOT NULL, descr VARCHAR(255) NOT NULL, sort_order INT NOT NULL DEFAULT 1, KEY idx_bet (bet_id), CONSTRAINT fk_bet FOREIGN KEY (bet_id) REFERENCES bets(id) ON DELETE CASCADE)"
    ];
